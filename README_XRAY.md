...
如果上述命令因网络或权限问题失败，您也可以使用传统的管道安装方式，但需要在安装后**手动运行 `xmanager` 命令**来启动菜单。
